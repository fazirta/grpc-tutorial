...
impl TransactionService for MyTransactionService {
    type GetTransactionHistoryStream =
        ReceiverStream<Result<TransactionResponse, Status>>;
...
impl ChatService for MyChatService {
    ...
    async fn chat(
        ...
        tokio::spawn(async move {
            while let Some(Ok(message)) = stream.message().await.transpose() {
                println!("Received message: {:?}", message);

                let reply = ChatMessage {
                    user_id: message.user_id.clone(),
                    message: format!(
                        "Terima kasih telah melakukan chat kepada CS virtual. Pesan anda: {}. Pesan akan dibalas pada jam kerja.",
                        message.message
                    ),
                };
